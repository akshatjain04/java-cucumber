...
Validation: 
This assertion verifies the correctness of the average calculation when all grades are positive numbers. It's significant because it validates the basic functionality of the getAverage method.
...
Validation: 
This assertion verifies that the getAverage method can handle the edge case where all grades are zero. This is significant because it ensures that the method doesn't produce an incorrect result in this scenario.

Scenario 3: Case where all grades are negative numbers
...
Assert: Use JUnit assertions to compare the actual result against the expected average.
Validation: 
This assertion verifies the correctness of the average calculation when all grades are negative numbers. It's significant because it ensures that the method can handle negative values correctly.

Scenario 4: Case where some grades are zero and others are positive numbers

Details:  
TestName: testGetAverageWithSomeZeroGrades
Description: This test checks the getAverage method when some of the grades (a, b, c) are zero and others are positive numbers. 
Execution:
Arrange: Set a, b and c with some zero and others positive numbers using the methods setA(), setB() and setC().
Act: Invoke the getAverage method.
Assert: Use JUnit assertions to compare the actual result against the expected average.
Validation: 
This assertion verifies the correctness of the average calculation when some grades are zero and others are positive numbers. It's significant because it tests the method's ability to handle a mix of zero and positive grades.
"""
*/
...
public class StudentGradeGetAverageTest {
    ...
    @Test
    @Tag("valid")
    public void testGetAverageWithSomeZeroGrades() {
        StudentGrade studentGrade = new StudentGrade();
        studentGrade.setA(0.0);
        studentGrade.setB(3.0);
        studentGrade.setC(5.0);
